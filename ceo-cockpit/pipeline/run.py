...
def main():
    ...
    sections.append(
        f"lead_classification split: top {n_pipeline} by final_score (locked-in cutoff) "
    ...
    sections.append("")
    sections.append(
        f"NEEDS ICP REVIEW ({len(result.needs_icp_review)} companies, missing ICP data -- "
        f"excluded from ranking/classification until scored, shown by raw_score)"
    )
    if not result.needs_icp_review.empty:
        rev = result.needs_icp_review[
            ["company_name", "raw_score", "icp_fit", "last_touchpoint_date", "touchpoint_count"]
        ].copy()
        rev["raw_score"] = rev["raw_score"].round(1)
        rev["last_touchpoint_date"] = rev["last_touchpoint_date"].dt.strftime("%Y-%m-%d")
        rev["note"] = "ranking provisional until ICP data is added"
        sections.append(rev.to_string(index=False))
    else:
